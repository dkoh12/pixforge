"""Command-line interface for pixforge."""
...
@click.option("--quality", "-q", default=85, show_default=True,
              help="Output quality (1-100, JPEG/WebP only)")
@click.option("--dpi", "-d", default=None, type=int,
              help="Set DPI (e.g. 300 for print)")
@click.option("--scale", "-s", default=None, type=float,
              help="Scale by percentage (e.g. 50 for 50%%)")
@click.option("--width", "-W", default=None, type=int,
              help="Resize to width (maintains aspect ratio if height omitted)")
@click.option("--height", "-H", default=None, type=int,
              help="Resize to height (maintains aspect ratio if width omitted)")
@click.option("--rotate", "-r", "rotation", default=None, type=int,
              help="Rotate by degrees (counter-clockwise)")
@click.option("--flip", "-f", "flip_dir", default=None,
              type=click.Choice(["horizontal", "vertical"]),
              help="Flip direction")
@click.option("--grayscale", "-g", "to_grayscale", is_flag=True,
              help="Convert to grayscale")
@click.option("--crop", "-c", nargs=4, type=int, metavar="X Y W H",
              default=None, help="Crop: x y width height")
def convert_cmd(  # pylint: disable=too-many-arguments,too-many-positional-arguments
    src, output, quality, dpi, scale, width, height, rotation, flip_dir, to_grayscale, crop
):
    ...
    img = Image.open(src)
    click.echo(f"📂 Input:  {src} ({img.size[0]}x{img.size[1]}, {img.mode})")

    img = apply_transforms(img, scale, width, height, crop_box=crop,
                           rotation=rotation, flip_dir=flip_dir, to_grayscale=to_grayscale)
...
@click.option("--format", "-F", "out_format", required=True,
              help="Output format (e.g. webp, jpg, png)")
@click.option("--quality", "-q", default=85, show_default=True,
              help="Output quality (1-100)")
@click.option("--scale", "-s", default=None, type=float,
              help="Scale by percentage")
...
@click.option("--grayscale", "-g", "to_grayscale", is_flag=True,
              help="Convert to grayscale")
def batch(  # pylint: disable=too-many-arguments,too-many-positional-arguments,too-many-locals
    input_dir, output_dir, out_format, quality, scale, width, height, to_grayscale
):
    ...
    for f in files:
        try:
            ...
            img = Image.open(f)
            img = apply_transforms(img, scale, width, height, to_grayscale=to_grayscale)
...
def gui(port, debug):
    ...
    from pixforge.gui.app import run  # pylint: disable=import-outside-toplevel
